...
namespace EscuelaConMaui.Services
{
    ...
    {
        List<TraerTodoDefinitivoModels> traerTodosDefinitivo = new();
        string? StudentName;
        ...
        public int existStudent(string studentName)
        {
            int existe = -1;
            for(int i = 0; i < traerTodosDefinitivo.Count; i++ )
            {
                if(traerTodosDefinitivo.Count > 0) //Siempre debo corroborar que la colección NO esté vacía. 
                {
                    if (traerTodosDefinitivo[i].NameAndLastName.Equals(studentName))
                    {
                        existe = i;
